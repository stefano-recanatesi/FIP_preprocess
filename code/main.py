...
import importlib
from util_data import preprocessing, load_session, dict2dataframe, plot_rawexample
...
for i_session in np.arange(len(folders_sessions))[:]:
    ...
    try:          
        ...
        if method2:
            events, traces, bit_code, df = new_function(i_session)

        if method3:
            df = new_function(i_session)
        ...
        # Plot behavior and traces if wanted
        # os.remove('/results/'+folder_session.split('/')[-1]+'/*')
        state = 'plotting'       
        try:
            plot_rawexample(events, traces_renamed, trace_times, time_span=[0,trace_times.max()], session_name=session_name, save=True, plot=False, tag='allsession')
            plot_rawexample(events, traces_renamed, trace_times, time_span=[trace_times.max()-600,trace_times.max()], session_name=session_name, save=True, plot=False, tag='endsession')
        except:
    ...
    # except TimeoutException:
        # print("Taking too long: " + folders_sessions[i_session])
        # files = glob.glob('/results/'+folder_session.split('/')[-1]+'/*')
        # for f in files:
            # os.remove(f)
        # continue
